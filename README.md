...
- nested `yield*` delegation inside bounded generator block bodies is supported for both sync and async generators, including async iterator-like objects whose `next()` returns a promise value.
