...
- nested expression wrapper for inline scripts:
  - `expr(...)`
- classic-JS inline script calls, scalar `??` nullish coalescing, decimal `BigInt` literals, local `let` / `const` bindings, logical assignment operators on local bindings, block-bodied `if` / `else` control flow, block-bodied `while` / `do...while` / `for` loops, bounded `switch` statements with `break`, bounded `try` / `catch` / `finally` blocks, class declarations with `static` blocks and public `static` fields, and nullish-base optional chaining between inline-script expressions:
  - `host.method(...)`
  - `host?.method(...)`
